'''
邢不行 | 量化小讲堂文章《BTC涨这么多，还能买吗？要卖吗？| 量化定投策略告诉你答案【附代码】》
微信号：xbx3642
'''
import pandas as pd
...
df = df[df['candle_time'] >= pd.to_datetime('20171215')]
...
df.to_csv('花式定投（D：%s,X:%s,Y:%s）.csv' % (days, region, step), encoding='gbk', index=False)
print(df.tail(5))
